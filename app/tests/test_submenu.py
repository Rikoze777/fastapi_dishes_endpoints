...
def test_get_empty_submenu(test_client, delete_menus):
    response = test_client.get(f"/api/v1/menus/{TEST_MENU_ID}/submenus")
...
def test_add_submenu(test_client, menu_id):
    ...
    response = test_client.post(f"/api/v1/menus/{menu_id}/submenus",
                           json=data)
    assert response.status_code == 201
    menu = response.json()
    assert menu["title"] == data["title"]
...
def test_update_submenu(test_client, menu_id, submenu_id):
    ...
    response = test_client.patch(f"/api/v1/menus/{menu_id}/submenus/{submenu_id}/",
                            json=data)
    assert response.status_code == 200
    menu = response.json()
    assert menu["title"] == data["title"]
...
def test_get_submenu_not_exists(test_client, delete_menus):
    response = test_client.get(f"/api/v1/menus/{TEST_MENU_ID}/submenus/{TEST_SUBMENU_ID}")
